...
func newTracker() *Tracker {
	return New(geo.Status{State: geo.StateStarting, Permission: geo.PermissionUnknown}, fixedclock.New(time.Now()))
}
...
func TestUnknownPermissionInheritsWhatWeAlreadyKnow(t *testing.T) {
	tracker := newTracker()
	if _, changed := tracker.Set(geo.Status{State: geo.StateReady, Permission: geo.PermissionGranted}); !changed {
...
func TestMarkReadySettlesThePermissionQuestion(t *testing.T) {
	for _, before := range []geo.PermissionState{geo.PermissionUnknown, geo.PermissionPromptRequired} {
		tracker := New(geo.Status{State: geo.StateStarting, Permission: before}, fixedclock.New(time.Now()))
		...
		if recorded.Permission != geo.PermissionGranted {
			t.Fatalf("permission = %v, want PermissionGranted: a fix cannot arrive without access", recorded.Permission)
		}
	}
}

func TestMarkReadyLeavesADeniedPermissionAlone(t *testing.T) {
	tracker := New(geo.Status{State: geo.StateDisabled, Permission: geo.PermissionRestricted}, fixedclock.New(time.Now()))
...
func TestNewStampsAnUnsetTime(t *testing.T) {
	if got := New(geo.Status{State: geo.StateStarting}, fixedclock.New(time.Now())).Get().UpdatedAt; got.IsZero() {

...
func factoryFor(p *fakeProvider, err error) provider.Factory {
	return provider.FactoryFunc(func(_ provider.Options, sink provider.Sink) (provider.Provider, error) {
		if err != nil {
			return nil, err
		}
		p.sink = sink
...
func TestOpenBoundsStartupWithStartTimeout(t *testing.T) {
	native := &fakeProvider{}
	loc, err := open(context.Background(), Config{StartTimeout: time.Minute}, factoryFor(native, nil), core.Features{})
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	defer loc.Close()
...
func TestOpenRejectsAnInvalidConfigBeforeTouchingTheProvider(t *testing.T) {
	native := &fakeProvider{}
	_, err := open(context.Background(), Config{MinimumDistanceMeters: -1}, factoryFor(native, nil), core.Features{})
...
func TestOpenRejectsANilContext(t *testing.T) {
	...
	if _, err := Open(nil, DefaultConfig()); !errors.Is(err, ErrInvalidConfig) {
...
func TestReExportedConstantsMatchTheirFeature(t *testing.T) {
	var (
		...
		_ SubscriptionConfig = SubscriptionConfig{DropPolicy: DropOldest, Buffer: 1}
		_ Accuracy           = AccuracyNavigation
		_ PermissionMode     = PermissionDoNotRequest
	)
